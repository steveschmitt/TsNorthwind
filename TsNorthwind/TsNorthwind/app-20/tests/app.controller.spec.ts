namespace app20 {
    ...
    describe('AppController', () => {
            ...
            beforeEach(inject((
                ...
                spyDataservice = {
                    ...
                    getAllCustomers: jasmine.createSpy('getAllCustomers')
                        .and.callFake(() => $q.when(mockCustomers)),
                    getCustomerById: jasmine.createSpy('getCustomerById')
                        .and.callFake(() => mockCustomers[0])
                };
        ...
        var mockBreezeCustomers: BreezeCustomer[];

        function resetMockBreezeCustomers() {
            mockBreezeCustomers = [
                newBreezeCustomer("a1", "Acme", "Aaron"),
                newBreezeCustomer("b1", "Bechtel", "Brian"),
                newBreezeCustomer("c1", "ConAgra", "Carl")
            ];
        }

        function newBreezeCustomer(id: string, companyName: string, contactName: string) {
            var bc = new BreezeCustomer();
            bc.customerID = id;
            bc.companyName = companyName;
            bc.contactName = contactName;
            return bc;
        }



        class HappyMockDataservice implements IBreezeDataservice {
            name = 'HappyMockDataservice';

            static $inject = ['$q'];
            constructor(public _$q: angular.IQService) { }

            getAllCustomers = jasmine.createSpy('getAllCustomers')
                .and.callFake(() => this._$q.when(mockBreezeCustomers));
            getCustomerPage = jasmine.createSpy('getCustomerPage')
                .and.callFake(() => this._$q.when(mockBreezeCustomers));
            getCustomerById = jasmine.createSpy('getCustomerById')
                .and.callFake(() => mockBreezeCustomers[0]);
            saveChanges = jasmine.createSpy('saveChanges')
                .and.callFake(() => this._$q.when(true));
        }

        var SERVER_FAILED_MSG = 'Server Failed: simulated error';
        var GETALL_FAILED_MSG = 'dataservice.getAllMethod threw: simulated error';

        class ThrowingMockDataservice extends HappyMockDataservice {
